...
function generateThemeObjectFromDaisyUI(themeString: string, themeName: string): ThemeMap {
  ...
  const theme = mapColors() as ThemeMap;

  // prefix
  for (const key of Object.keys(theme)) {
    const colorSet = theme[key];
    for (const colorKey of Object.keys(colorSet)) {
      const value = colorSet[colorKey];
      if (typeof value === 'string' && value.startsWith('oklch')) {
        colorSet[colorKey] = 'oklchToHex(' + value + ')';
      }
    }
  }
